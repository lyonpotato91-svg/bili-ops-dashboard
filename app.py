...
def fetch_video_stats_by_bvid(bvid: str) -> dict:
    ...
    """
    # 常见的公开视频信息接口思路（非官方封装/反向接口可能变化）
    api = "https://api.bilibili.com/x/web-interface/view"
    r = requests.get(api, params={"bvid": bvid}, timeout=10)
    data = r.json()
    if data.get("code") != 0:
...
def compute_metrics(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["deep_signal_ratio"] = np.where(df["engagement"] > 0, (df["coin"] + df["favorite"]) / df["engagement"], 0.0)
    return df
...
# 走势图（按抓取时间展示；你后续可做“每日快照”才是真正的趋势）
